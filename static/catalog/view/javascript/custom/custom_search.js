$(document).ready(function() {
	$(".custom-search input[name='search']").autocomplete({
		source: function(request, response) {
			const categoryId = ($('#customSearch select[name=\'category_id\']').val()) ? Number($('#customSearch select[name=\'category_id\']').val()) : 0;
			const url = "index.php?route=extension/module/custom_search&filter_name=" + request.term + "&filter_category=" + categoryId;
			$.ajax({
				url,
				data: {
	...
	$.ui.autocomplete.prototype._renderItem = function(ul, item) {
		...
		return $("<li></li>")
			...
			.append('<a href="'+ item.value +'" class="custom-search clearfix">' + elements_html + '</a>')
			.appendTo(ul);
	};
});
